...
class EnrollmentTest < Minitest::Test
  ...
  assert_in_delta 0.144, enrollment.kindergarten_participation_in_year(2004), 0.005
    ...
    def test_it_can_load_a_second_data_file
      er = EnrollmentRepository.new
      er.load_data({
        :enrollment => {
          :kindergarten => "./data/Kindergartners in full-day program.csv",
    ...
    def test_it_can_return_graduation_rate_by_year
      er = EnrollmentRepository.new
      er.load_data({
        :enrollment => {
          :kindergarten => "./data/Kindergartners in full-day program.csv",
      ...
      enrollment = er.find_by_name("ACADEMY 20")
      assert_equal enrollment.graduation_rate_by_year, { 2010 => 0.895, 2011 => 0.895, 2012 => 0.889, 2013 => 0.913, 2014 => 0.898}
    end

    def test_it_can_return_graduation_rate_in_specific_year
      er = EnrollmentRepository.new
      er.load_data({
        :enrollment => {
          :kindergarten => "./data/Kindergartners in full-day program.csv",
